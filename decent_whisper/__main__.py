...
from decent_whisper import available_models, get_backend, transcribe, available_backends
...
parser.add_argument('--benchmark', help='benechmark all available models', default=False, action='store_true')
...
if args.benchmark:
    print(f"will benchmark models: {', '.join([model.name() for model in available_backends()])}")
    backends = available_backends()

for backend in backends:
    log(f"-> transcribing with backend {backend.name()}")

    if args.model is not None:
        models = [model for model in backend.available_models() if model.name == args.model]
        if len(models) > 0:
            model_info = models[0]
    else:
